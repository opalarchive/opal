...
export const execute = async (req, res) => {
  ...
  Object.keys(projectPublic.editors).forEach(async (editor) => {
    let currentNotifications =
      (await db
        .ref(`userInformation/${editor}/notifications`)
        .once("value")
        .then((snapshot) => snapshot.val())) || {};
    ...
    currentNotifications.push({
      content: `${ownerUsername} has deleted <a href="/project/view/${uuid}">${projectPublic.name}</a>! The OPAL project will now only be view-only until the owner chooses to restore it.`,
